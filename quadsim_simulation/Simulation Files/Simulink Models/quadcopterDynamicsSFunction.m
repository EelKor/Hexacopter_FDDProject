...
function setup(block)
  ...
  block.InputPort(8).Dimensions        = 6; % each motor's fault
...
%% *************************************
%% gyroscopic torque created by motors
tau_motorGyro = [Q*quad.Jm*2*pi/60*(-w1-w3-w5+w2+w4+w6); P*quad.Jm*2*pi/60*(w1+w3+w5-w2-w4-w6); 0]; % Note: 2*pi/60 required to convert from RPM to radians/s
%% *************************************
%% hexacopter mixing
d1 = quad.d*cos(60*pi/180);
d2 = quad.d*sin(60*pi/180);
ct = quad.ct*fault;
cq = quad.cq*fault;
% ct = quad.ct*fault;
% cq = quad.cq*fault;
...
A_mixing = [
    -d1*ct(1) d1*ct(2) quad.d*ct(3) d1*ct(4) -d1*ct(5) -quad.d*ct(6)
    -d2*ct(1) -d2*ct(2) 0 d2*ct(4) d2*ct(5) 0
    -cq(1) cq(2) -cq(3) cq(4) -cq(5) cq(6)  
    ];
%% torque
% Mb = (quad.dctcq*(w.^2))+ tau_motorGyro + (Dist_tau);  
Mb = (A_mixing*(w.^2))+ tau_motorGyro + (Dist_tau); % Mb = [tau1 tau2 tau3]'
...
Fb = [0; 0; sum(ct.*(w.^2))];   %[0, 0, sum(ct*w.^2)]'
%% Obtain dP dQ dR
omb_bi = [P; Q; R];
OMb_bi = [ 0,-R, Q;
           R, 0,-P;
