...
void *subrateTask(void *arg)
{
  ...
  while (runModel) {
    ...
    FLIGHT_Hexacopter_V2_1_step(subRateId);
...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(FLIGHT_Hexacopter_V2_1_M) == (NULL));
  while (runModel) {
    ...
    if (rtmStepTask(FLIGHT_Hexacopter_V2_1_M, 1)
        ) {
      px4_sem_post(&subrateTaskSem[0]);
    }

    FLIGHT_Hexacopter_V2_1_step(0);

    // Get model outputs here
    stopRequested = !((rtmGetErrorStatus(FLIGHT_Hexacopter_V2_1_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(FLIGHT_Hexacopter_V2_1_M, "stopping the model");
  runModel = 0;
}
...
void *terminateTask(void *arg)
{
  ...
  // Terminate model
  FLIGHT_Hexacopter_V2_1_terminate();
...
int px4_simulink_app_task_main (int argc, char *argv[])
{
  ...
  rtmSetErrorStatus(FLIGHT_Hexacopter_V2_1_M, 0);

  // Initialize model
  FLIGHT_Hexacopter_V2_1_initialize();
