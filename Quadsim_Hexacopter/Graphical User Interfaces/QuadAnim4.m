...
ang = 30*pi/180; % Hexacopter
r1 = [d*cos(ang) -d*sin(ang) h].';% m1 rotor center [X Y Z]
r2 = [d*cos(ang) d*sin(ang) h].';% m1 rotor center [X Y Z]
r3 = [0 d h].';% m1 rotor center [X Y Z]
r4 = [-d*cos(ang) d*sin(ang) h].';% m1 rotor center [X Y Z]
r5 = [-d*cos(ang) -d*sin(ang) h].';% m1 rotor center [X Y Z]
r6 = [0 -d h].';% m1 rotor center [X Y Z]
r1r = circlePoints(r1, r, 10); r1r = [r1r r1r(:,1)]; % Rotor blade circles
r2r = circlePoints(r2, r, 10); r2r = [r2r r2r(:,1)]; % Rotor blade circles
r3r = circlePoints(r3, r, 10); r3r = [r3r r3r(:,1)]; % Rotor blade circles
r4r = circlePoints(r4, r, 10); r4r = [r4r r4r(:,1)]; % Rotor blade circles
r5r = circlePoints(r5, r, 10); r5r = [r5r r5r(:,1)]; % Rotor blade circles
r6r = circlePoints(r6, r, 10); r6r = [r6r r6r(:,1)]; % Rotor blade circles
...
%     d, -d;
%     0,  0];
b14 = [ d*cos(ang), -d*cos(ang);
    -d*sin(ang),  d*sin(ang);
    0,  0]; % For drawing the body "X" shape
b25 = [ d*cos(ang), -d*cos(ang);
    d*sin(ang),  -d*sin(ang);
    0,  0]; % For drawing the body "X" shape
b36 = [ 0, 0;
    d,  -d;
...
% vector
axes(handles.axes1)
plot3(b14R(1,:),b14R(2,:),b14R(3,:),'b','LineWidth',3) % Body Arm
hold on
plot3(b25R(1,:),b25R(2,:),b25R(3,:),'b','LineWidth',3) % Body Arm
hold on
plot3(b36R(1,:),b36R(2,:),b36R(3,:),'b','LineWidth',3) % Body Arm
% xlim([-100 100])
% ylim([-100 100])
% zlim([-100 100])
%% Hexacopter!!!!
plot3(m1r(1,:),m1r(2,:),m1r(3,:),'k','LineWidth',4) % Motor
plot3(m2r(1,:),m2r(2,:),m2r(3,:),'k','LineWidth',4) % Motor
plot3(m3r(1,:),m3r(2,:),m3r(3,:),'k','LineWidth',4) % Motor
plot3(m4r(1,:),m4r(2,:),m4r(3,:),'k','LineWidth',4) % Motor
plot3(m5r(1,:),m5r(2,:),m5r(3,:),'k','LineWidth',4) % Motor
plot3(m6r(1,:),m6r(2,:),m6r(3,:),'k','LineWidth',4) % Motor

plot3(r1R(1,:),r1R(2,:),r1R(3,:),'g') % Motor 1 blades
plot3(r2R(1,:),r2R(2,:),r2R(3,:),'k') % Motor 2 blades
plot3(r3R(1,:),r3R(2,:),r3R(3,:),'g') % Motor 3 blades
plot3(r4R(1,:),r4R(2,:),r4R(3,:),'k') % Motor 4 blades
plot3(r5R(1,:),r5R(2,:),r5R(3,:),'g') % Motor 5 blades
plot3(r6R(1,:),r6R(2,:),r6R(3,:),'k') % Motor 6 blades
