...
// System initialize for atomic system:
void FLIGHT_hexa_PX4Timestamp_f_Init(DW_PX4Timestamp_FLIGHT_hexa_b_T *localDW)
{
  // Start for MATLABSystem: '<S567>/PX4 Timestamp'
  localDW->obj.matlabCodegenIsDeleted = false;
  localDW->objisempty = true;
  localDW->obj.isInitialized = 1;
  localDW->obj.isSetupComplete = true;
...
// Output and update for atomic system:
void FLIGHT_hexacopte_PX4Timestamp_o(B_PX4Timestamp_FLIGHT_hexac_l_T *localB)
{
  // MATLABSystem: '<S567>/PX4 Timestamp'
  localB->PX4Timestamp = hrt_absolute_time();
}

// Termination for atomic system:
void FLIGHT_hexa_PX4Timestamp_m_Term(DW_PX4Timestamp_FLIGHT_hexa_b_T *localDW)
{
  // Terminate for MATLABSystem: '<S567>/PX4 Timestamp'
  if (!localDW->obj.matlabCodegenIsDeleted) {
    localDW->obj.matlabCodegenIsDeleted = true;
  }

  // End of Terminate for MATLABSystem: '<S567>/PX4 Timestamp'
}
...
void FLIGHT_hexacopter_step(void)
{
  ...
  // Outputs for Enabled SubSystem: '<S412>/Enabled Subsystem' incorporates:
  //   EnablePort: '<S413>/Enable'

  // Start for MATLABSystem: '<S412>/SourceBlock'
  if (b_varargout_1) {
    // SignalConversion generated from: '<S413>/In1'
    FLIGHT_hexacopter_B.In1_n = FLIGHT_hexacopter_B.r2;
  }

  // End of Outputs for SubSystem: '<S412>/Enabled Subsystem'

  // RelationalOperator: '<S411>/Compare' incorporates:
  //   Constant: '<S411>/Constant'
  //   SignalConversion generated from: '<S410>/Bus Selector'
  ...
  if (b_varargout_1) {
    // SignalConversion generated from: '<S404>/In1'
  ...
  if ((FLIGHT_hexacopter_B.In1_n.values[5] > 1400) &&
    ...
    if (rtb_Compare) {
      ...
      rtb_IProdOut_p *= FLIGHT_hexacopter_B.ReadParameter2_pna.ReadParameter2_o1;
  ...
  // Start for MATLABSystem: '<S406>/SourceBlock'
  if (b_varargout_1) {
  ...
  //  release.
  rtb_Compare_o = (FLIGHT_hexacopter_B.In1_n.values[6] >=
  ...
  FLIGHT_hexacopter_B.out_yawrate = FLIGHT_hexacopter_B.Merge1[2];
  ...
  //  Define Doublet
  FLIGHT_hexacopter_B.peak_angle_rad = rtb_Saturation3 * 0.017453292519943295;

  //  Default Output
  //  Generate Doublet Command
  if (rtb_Compare_o) {
    switch (FLIGHT_hexacopter_B.ParamStep) {
     case 1:
      if ((FLIGHT_hexacopter_B.reletive_time_sec >= 0.0) &&
          (FLIGHT_hexacopter_B.reletive_time_sec < rtb_IProdOut_p)) {
        FLIGHT_hexacopter_B.out_roll = FLIGHT_hexacopter_B.peak_angle_rad;
      } else if ((FLIGHT_hexacopter_B.reletive_time_sec >= rtb_IProdOut_p) &&
                 (FLIGHT_hexacopter_B.reletive_time_sec < static_cast<real_T>
                  (rtb_IProdOut_p) + rtb_IProdOut_p)) {
        FLIGHT_hexacopter_B.out_roll = -FLIGHT_hexacopter_B.peak_angle_rad;
      } else {
  ...
  if (rtb_Compare) {
    ...
    //   Sum: '<S2>/Sum3'
    //
    FLIGHT_hexacopter_B.Switch2 = (static_cast<real32_T>
      (FLIGHT_hexacopter_B.out_roll) - rt_atan2f_snf
    ...
    //   Sum: '<S2>/Sum2'
    //
    FLIGHT_hexacopter_B.Switch2_i = (static_cast<real32_T>
      (FLIGHT_hexacopter_B.out_pitch) - static_cast<real32_T>(asin
      (static_cast<real_T>((FLIGHT_hexacopter_B.In1_k.q[1] *
      FLIGHT_hexacopter_B.In1_k.q[3] - FLIGHT_hexacopter_B.In1_k.q[0] *
      FLIGHT_hexacopter_B.In1_k.q[2]) * -2.0F)))) * rtb_Saturation3;
  ...
  if (rtb_Compare) {
    ...
    // Product: '<S503>/NProd Out' incorporates:
    //   Constant: '<S9>/Constant'
    //   DiscreteIntegrator: '<S495>/Filter'
    //   MATLABSystem: '<S9>/Read Parameter2'
    //   Product: '<S493>/DProd Out'
    //   Sum: '<S495>/SumD'
    //
    ...
    // Sum: '<S509>/Sum' incorporates:
    //   DiscreteIntegrator: '<S500>/Integrator'
    //   MATLABSystem: '<S9>/Read Parameter'
    //   Product: '<S505>/PProd Out'
    //
    FLIGHT_hexacopter_B.Sum = (rtb_IProdOut_p * rtb_Saturation3 +
    ...
    // Product: '<S497>/IProd Out' incorporates:
    //   MATLABSystem: '<S9>/Read Parameter1'
    //
    ...
    // Product: '<S453>/NProd Out' incorporates:
    //   Constant: '<S9>/Constant1'
    //   DiscreteIntegrator: '<S445>/Filter'
    //   MATLABSystem: '<S9>/Read Parameter5'
    //   Product: '<S443>/DProd Out'
    //   Sum: '<S445>/SumD'
    //
    ...
    rtb_IProdOut_p = static_cast<real32_T>(FLIGHT_hexacopter_B.out_yawrate) -
    ...
    // Product: '<S553>/NProd Out' incorporates:
    //   Constant: '<S9>/Constant2'
    //   DiscreteIntegrator: '<S545>/Filter'
    //   MATLABSystem: '<S9>/Read Parameter8'
    //   Product: '<S543>/DProd Out'
    //   Sum: '<S545>/SumD'
    //
    ...
    // Update for DiscreteIntegrator: '<S495>/Filter'
    FLIGHT_hexacopter_DW.Filter_DSTATE += FLIGHT_hexacopter_P.Filter_gainval_o *
      rtb_SinCos_o1;

    // Update for DiscreteIntegrator: '<S500>/Integrator'
  ...
  for (FLIGHT_hexacopter_B.ParamStep = 0; FLIGHT_hexacopter_B.ParamStep < 6;
       FLIGHT_hexacopter_B.ParamStep++) {
    FLIGHT_hexacopter_B.DataTypeConversion6[FLIGHT_hexacopter_B.ParamStep] =
      FLIGHT_hexacopter_B.Output_Limits1[FLIGHT_hexacopter_B.ParamStep];
  }
  ...
  rtb_Compare_o = (FLIGHT_hexacopter_B.In1_n.values[7] >=
  ...
  b_varargout_1 = MW_Param_Step(FLIGHT_hexacopter_DW.obj_j5.MW_PARAMHANDLE,
    MW_INT32, &FLIGHT_hexacopter_B.ParamStep_m);
  if (b_varargout_1) {
    FLIGHT_hexacopter_B.ParamStep_m = 0;
  }
  ...
  //
  if (rtb_Compare_o) {
    FLIGHT_hexacopter_B.reletive_time_sec = FLIGHT_hexacopter_B.ParamStep;
  } else {
    FLIGHT_hexacopter_B.reletive_time_sec =
      FLIGHT_hexacopter_P.Constant_Value_mc;
  }

  // Gain: '<S181>/Gain' incorporates:
  //   Switch: '<S4>/Switch'

  FLIGHT_hexacopter_B.reletive_time_sec *= FLIGHT_hexacopter_P.Gain_Gain_j;

  // Saturate: '<S181>/Saturation'
  if (FLIGHT_hexacopter_B.reletive_time_sec >
      FLIGHT_hexacopter_P.Saturation_UpperSat) {
    FLIGHT_hexacopter_B.reletive_time_sec =
      FLIGHT_hexacopter_P.Saturation_UpperSat;
  ...
  FLIGHT_hexacopter_B.reletive_time_sec = floor((static_cast<real_T>
    (FLIGHT_hexacopter_B.DataTypeConversion6[0]) -
    FLIGHT_hexacopter_P.Constant_Value_kf) *
    FLIGHT_hexacopter_B.reletive_time_sec +
    FLIGHT_hexacopter_P.Constant_Value_kf);
  if (rtIsNaN(FLIGHT_hexacopter_B.reletive_time_sec) || rtIsInf
      (FLIGHT_hexacopter_B.reletive_time_sec)) {
    FLIGHT_hexacopter_B.reletive_time_sec = 0.0;
  } else {
    FLIGHT_hexacopter_B.reletive_time_sec = fmod
      (FLIGHT_hexacopter_B.reletive_time_sec, 65536.0);
  }

  FLIGHT_hexacopter_B.DataTypeConversion7_k[0] = static_cast<uint16_T>
    (FLIGHT_hexacopter_B.reletive_time_sec < 0.0 ? static_cast<int32_T>(
      static_cast<uint16_T>(-static_cast<int16_T>(static_cast<uint16_T>
        (-FLIGHT_hexacopter_B.reletive_time_sec)))) : static_cast<int32_T>(
      static_cast<uint16_T>(FLIGHT_hexacopter_B.reletive_time_sec)));
  ...
  //
  if (rtb_Compare_o) {
    FLIGHT_hexacopter_B.reletive_time_sec = FLIGHT_hexacopter_B.ParamStep_m;
  } else {
    FLIGHT_hexacopter_B.reletive_time_sec = FLIGHT_hexacopter_P.Constant1_Value;
  }

  // Gain: '<S182>/Gain' incorporates:
  //   Switch: '<S4>/Switch1'

  FLIGHT_hexacopter_B.reletive_time_sec *= FLIGHT_hexacopter_P.Gain_Gain_h;

  // Saturate: '<S182>/Saturation'
  if (FLIGHT_hexacopter_B.reletive_time_sec >
      FLIGHT_hexacopter_P.Saturation_UpperSat_m) {
    FLIGHT_hexacopter_B.reletive_time_sec =
      FLIGHT_hexacopter_P.Saturation_UpperSat_m;
  } else if (FLIGHT_hexacopter_B.reletive_time_sec <
  ...
  //
  if (rtb_Compare_o) {
    FLIGHT_hexacopter_B.reletive_time_sec = FLIGHT_hexacopter_B.ParamStep_c;
  } else {
    FLIGHT_hexacopter_B.reletive_time_sec = FLIGHT_hexacopter_P.Constant2_Value;
  }

  // Gain: '<S183>/Gain' incorporates:
  //   Switch: '<S4>/Switch2'

  FLIGHT_hexacopter_B.reletive_time_sec *= FLIGHT_hexacopter_P.Gain_Gain_c;

  // Saturate: '<S183>/Saturation'
  if (FLIGHT_hexacopter_B.reletive_time_sec >
      FLIGHT_hexacopter_P.Saturation_UpperSat_b) {
    FLIGHT_hexacopter_B.reletive_time_sec =
      FLIGHT_hexacopter_P.Saturation_UpperSat_b;
  } else if (FLIGHT_hexacopter_B.reletive_time_sec <
  ...
  //
  if (rtb_Compare_o) {
    FLIGHT_hexacopter_B.reletive_time_sec = ParamStep;
  } else {
    FLIGHT_hexacopter_B.reletive_time_sec = FLIGHT_hexacopter_P.Constant3_Value;
  }

  // Gain: '<S184>/Gain' incorporates:
  //   Switch: '<S4>/Switch3'

  FLIGHT_hexacopter_B.reletive_time_sec *= FLIGHT_hexacopter_P.Gain_Gain_n;

  // Saturate: '<S184>/Saturation'
  if (FLIGHT_hexacopter_B.reletive_time_sec >
      FLIGHT_hexacopter_P.Saturation_UpperSat_be) {
    FLIGHT_hexacopter_B.reletive_time_sec =
      FLIGHT_hexacopter_P.Saturation_UpperSat_be;
  } else if (FLIGHT_hexacopter_B.reletive_time_sec <
  ...
  //
  if (rtb_Compare_o) {
    FLIGHT_hexacopter_B.reletive_time_sec = ParamStep_0;
  } else {
    FLIGHT_hexacopter_B.reletive_time_sec = FLIGHT_hexacopter_P.Constant4_Value;
  }

  // Gain: '<S185>/Gain' incorporates:
  //   Switch: '<S4>/Switch4'

  FLIGHT_hexacopter_B.reletive_time_sec *= FLIGHT_hexacopter_P.Gain_Gain_f;

  // Saturate: '<S185>/Saturation'
  if (FLIGHT_hexacopter_B.reletive_time_sec >
      FLIGHT_hexacopter_P.Saturation_UpperSat_d) {
    FLIGHT_hexacopter_B.reletive_time_sec =
      FLIGHT_hexacopter_P.Saturation_UpperSat_d;
  } else if (FLIGHT_hexacopter_B.reletive_time_sec <
  ...
  FLIGHT_hexacopter_B.reletive_time_sec = floor((static_cast<real_T>
    ...
    FLIGHT_hexacopter_B.reletive_time_sec + FLIGHT_hexacopter_P.Constant_Value_l);
  if (rtIsNaN(FLIGHT_hexacopter_B.reletive_time_sec) || rtIsInf
      (FLIGHT_hexacopter_B.reletive_time_sec)) {
    FLIGHT_hexacopter_B.reletive_time_sec = 0.0;
  } else {
  ...
  //
  if (rtb_Compare_o) {
    FLIGHT_hexacopter_B.reletive_time_sec = ParamStep_1;
  } else {
    FLIGHT_hexacopter_B.reletive_time_sec = FLIGHT_hexacopter_P.Constant5_Value;
  }

  // Gain: '<S186>/Gain' incorporates:
  //   Switch: '<S4>/Switch5'

  FLIGHT_hexacopter_B.reletive_time_sec *= FLIGHT_hexacopter_P.Gain_Gain_m;

  // Saturate: '<S186>/Saturation'
  if (FLIGHT_hexacopter_B.reletive_time_sec >
      FLIGHT_hexacopter_P.Saturation_UpperSat_i) {
    FLIGHT_hexacopter_B.reletive_time_sec =
      FLIGHT_hexacopter_P.Saturation_UpperSat_i;
  } else if (FLIGHT_hexacopter_B.reletive_time_sec <
  ...
  FLIGHT_hexacopter_B.reletive_time_sec = floor((static_cast<real_T>
    ...
    FLIGHT_hexacopter_B.reletive_time_sec + FLIGHT_hexacopter_P.Constant_Value_o);
  if (rtIsNaN(FLIGHT_hexacopter_B.reletive_time_sec) || rtIsInf
      (FLIGHT_hexacopter_B.reletive_time_sec)) {
    FLIGHT_hexacopter_B.reletive_time_sec = 0.0;
  } else {
  ...
  for (FLIGHT_hexacopter_B.ParamStep = 0; FLIGHT_hexacopter_B.ParamStep < 8;
       FLIGHT_hexacopter_B.ParamStep++) {
    FLIGHT_hexacopter_B.pwmValue[FLIGHT_hexacopter_B.ParamStep] = 0U;
  }
  ...
  rtb_NProdOut_g = FLIGHT_hexacopter_P.u2_Gain * static_cast<real32_T>
    (FLIGHT_hexacopter_B.out_pitch);
  rtb_Saturation3 = FLIGHT_hexacopter_P.u2_Gain * static_cast<real32_T>
  ...
  FLIGHT_hexacopte_PX4Timestamp_o(&FLIGHT_hexacopter_B.PX4Timestamp_pn);
  ...
  tmp_3 = rtb_IProdOut_p * rtb_NProdOut_g;
  ...
  FLIGHT_hexacopter_B.BusAssignment_d.q_d[2] = tmp_3 * rtb_IProdOut_c +
  ...
  FLIGHT_hexacopte_PX4Timestamp_o(&FLIGHT_hexacopter_B.PX4Timestamp_ok);

  // BusAssignment: '<S572>/Bus Assignment' incorporates:
  //   Constant: '<S12>/Constant'
  //   Constant: '<S12>/Constant2'
  //   DataTypeConversion: '<Root>/Data Type Conversion1'
  //   MATLABSystem: '<S572>/PX4 Timestamp'
  ...
  for (FLIGHT_hexacopter_B.ParamStep = 0; FLIGHT_hexacopter_B.ParamStep < 6;
       FLIGHT_hexacopter_B.ParamStep++) {
    FLIGHT_hexacopter_B.BusAssignment.output[FLIGHT_hexacopter_B.ParamStep] =
      FLIGHT_hexacopter_B.DataTypeConversion7_k[FLIGHT_hexacopter_B.ParamStep];
  }

  for (FLIGHT_hexacopter_B.ParamStep = 0; FLIGHT_hexacopter_B.ParamStep < 10;
       FLIGHT_hexacopter_B.ParamStep++) {
    FLIGHT_hexacopter_B.BusAssignment.output[FLIGHT_hexacopter_B.ParamStep + 6] =
      FLIGHT_hexacopter_P.Constant_Value_bm[FLIGHT_hexacopter_B.ParamStep];
  }
  ...
  // End of BusAssignment: '<S572>/Bus Assignment'

  // MATLABSystem: '<S576>/SinkBlock' incorporates:
  ...
  FLIGHT_hexacopte_PX4Timestamp_o(&FLIGHT_hexacopter_B.PX4Timestamp_n);
  FLIGHT_hexacopte_PX4Timestamp_o(&FLIGHT_hexacopter_B.PX4Timestamp_p);

  // BusAssignment: '<S571>/Bus Assignment' incorporates:
  //   Constant: '<S570>/Constant'
  //   Gain: '<S570>/Gain'
  //   MATLABSystem: '<S12>/PX4 Timestamp'
  //   MATLABSystem: '<S571>/PX4 Timestamp'
  ...
  for (FLIGHT_hexacopter_B.ParamStep = 0; FLIGHT_hexacopter_B.ParamStep < 6;
    ...
    if (FLIGHT_hexacopter_B.ParamStep_m >
      ...
      rtb_Saturation3 = static_cast<real32_T>(FLIGHT_hexacopter_B.ParamStep_m);
    }

    FLIGHT_hexacopter_B.BusAssignment_f.control[FLIGHT_hexacopter_B.ParamStep] =
      (rtb_Saturation3 - FLIGHT_hexacopter_P.Constant_Value_ez) *
      FLIGHT_hexacopter_P.Gain_Gain_mg;

    // Saturate: '<S570>/Saturation' incorporates:
    ...
    rtb_IProdOut_p =
      FLIGHT_hexacopter_P.Constant1_Value_i[FLIGHT_hexacopter_B.ParamStep];
    ...
    FLIGHT_hexacopter_B.BusAssignment_f.control[FLIGHT_hexacopter_B.ParamStep +
      6] = (rtb_IProdOut_p - FLIGHT_hexacopter_P.Constant_Value_ez) *
      FLIGHT_hexacopter_P.Gain_Gain_mg;
    FLIGHT_hexacopter_B.BusAssignment_f._padding0[FLIGHT_hexacopter_B.ParamStep]
      = 0U;
  }

  // End of BusAssignment: '<S571>/Bus Assignment'

  // MATLABSystem: '<S574>/SinkBlock' incorporates:
  //   BusAssignment: '<S571>/Bus Assignment'
  ...
  FLIGHT_hexacopte_PX4Timestamp_o(&FLIGHT_hexacopter_B.PX4Timestamp_o);

  // BusAssignment: '<S567>/Bus Assignment' incorporates:
  //   MATLABSystem: '<S567>/PX4 Timestamp'
  ...
  // MATLABSystem: '<S569>/SinkBlock' incorporates:
  //   BusAssignment: '<S567>/Bus Assignment'
  ...
  // MATLABSystem: '<S585>/SinkBlock' incorporates:
  //   BusAssignment: '<S17>/Bus Assignment'
  ...
  FLIGHT_hexacopte_PX4Timestamp_o(&FLIGHT_hexacopter_B.PX4Timestamp_pnae);
...
void FLIGHT_hexacopter_initialize(void)
{
  ...
  {
    static const char_T ParameterNameStr[17] = "DOUBLET_PEAK_ANG";
    static const char_T ParameterNameStr_0[11] = "DOUBLET_DT";
    static const char_T ParameterNameStr_1[13] = "DOUBLET_AXIS";
    ...
    static const char_T ParameterNameStr_k[13] = "MC_YAWRATE_I";
    static const char_T ParameterNameStr_l[9] = "MPC_XY_P";
    static const char_T ParameterNameStr_m[17] = "MPC_XY_VEL_P_ACC";
    static const char_T ParameterNameStr_n[8] = "MPC_Z_P";
    real_T tmp;
    int32_T i;

    // SystemInitialize for Enabled SubSystem: '<S412>/Enabled Subsystem'
    // SystemInitialize for SignalConversion generated from: '<S413>/In1' incorporates:
    //   Outport: '<S413>/Out1'

    FLIGHT_hexacopter_B.In1_n = FLIGHT_hexacopter_P.Out1_Y0_p;

    // End of SystemInitialize for SubSystem: '<S412>/Enabled Subsystem'

    // SystemInitialize for Enabled SubSystem: '<S401>/Enabled Subsystem'
    // SystemInitialize for SignalConversion generated from: '<S404>/In1' incorporates:
    //   Outport: '<S404>/Out1'
    ...
    // SystemInitialize for Enabled SubSystem: '<S406>/Enabled Subsystem'
    ...
    // End of SystemInitialize for SubSystem: '<S406>/Enabled Subsystem'
    ...
    // SystemInitialize for Product: '<S166>/PProd Out' incorporates:
    //   Outport: '<S2>/des_p'
    //   Switch: '<S169>/Switch2'

    FLIGHT_hexacopter_B.Switch2 = FLIGHT_hexacopter_P.des_p_Y0;

    // SystemInitialize for Product: '<S115>/PProd Out' incorporates:
    //   Outport: '<S2>/des_q'
    //   Switch: '<S118>/Switch2'
    ...
    // InitializeConditions for DiscreteIntegrator: '<S495>/Filter'
    FLIGHT_hexacopter_DW.Filter_DSTATE =
    ...
    // InitializeConditions for DiscreteIntegrator: '<S445>/Filter'
    FLIGHT_hexacopter_DW.Filter_DSTATE_j =
    ...
    // InitializeConditions for DiscreteIntegrator: '<S545>/Filter'
    FLIGHT_hexacopter_DW.Filter_DSTATE_h =
    ...
    FLIGHT_hexacopter_DW.obj.MW_PARAMHANDLE = MW_Init_Param(&ParameterNameStr_i
      [0], true, tmp * 1000.0);
    ...
    // SystemInitialize for Sum: '<S509>/Sum' incorporates:
    //   Outport: '<S9>/tau_roll'

    FLIGHT_hexacopter_B.Sum = FLIGHT_hexacopter_P.tau_roll_Y0;

    // SystemInitialize for Sum: '<S459>/Sum' incorporates:
    //   Outport: '<S9>/tau_pitch'

    FLIGHT_hexacopter_B.Sum_a = FLIGHT_hexacopter_P.tau_pitch_Y0;

    // SystemInitialize for Sum: '<S559>/Sum' incorporates:
    //   Outport: '<S9>/tau_yaw'
    ...
    FLIGHT_hexa_PX4Timestamp_f_Init(&FLIGHT_hexacopter_DW.PX4Timestamp_pn);
    ...
    FLIGHT_hexa_PX4Timestamp_f_Init(&FLIGHT_hexacopter_DW.PX4Timestamp_ok);

    // Start for MATLABSystem: '<S576>/SinkBlock' incorporates:
    //   BusAssignment: '<S572>/Bus Assignment'
    ...
    FLIGHT_hexa_PX4Timestamp_f_Init(&FLIGHT_hexacopter_DW.PX4Timestamp_n);
    FLIGHT_hexa_PX4Timestamp_f_Init(&FLIGHT_hexacopter_DW.PX4Timestamp_p);
    ...
    FLIGHT_hexa_PX4Timestamp_f_Init(&FLIGHT_hexacopter_DW.PX4Timestamp_o);

    // Start for MATLABSystem: '<S569>/SinkBlock' incorporates:
    //   BusAssignment: '<S567>/Bus Assignment'
    ...
    // Start for MATLABSystem: '<S585>/SinkBlock' incorporates:
    //   BusAssignment: '<S17>/Bus Assignment'
    ...
    FLIGHT_hexa_PX4Timestamp_f_Init(&FLIGHT_hexacopter_DW.PX4Timestamp_pnae);
...
{
  // Terminate for MATLABSystem: '<S412>/SourceBlock'
  ...
  // End of Terminate for MATLABSystem: '<S412>/SourceBlock'
  ...
  // Terminate for MATLABSystem: '<S406>/SourceBlock'
  ...
  // End of Terminate for MATLABSystem: '<S406>/SourceBlock'
  ...
  FLIGHT_hexa_PX4Timestamp_m_Term(&FLIGHT_hexacopter_DW.PX4Timestamp_pn);
  ...
  FLIGHT_hexa_PX4Timestamp_m_Term(&FLIGHT_hexacopter_DW.PX4Timestamp_ok);

  // Terminate for MATLABSystem: '<S576>/SinkBlock'
  ...
  // End of Terminate for MATLABSystem: '<S576>/SinkBlock'
  FLIGHT_hexa_PX4Timestamp_m_Term(&FLIGHT_hexacopter_DW.PX4Timestamp_n);
  FLIGHT_hexa_PX4Timestamp_m_Term(&FLIGHT_hexacopter_DW.PX4Timestamp_p);
  ...
  FLIGHT_hexa_PX4Timestamp_m_Term(&FLIGHT_hexacopter_DW.PX4Timestamp_o);

  // Terminate for MATLABSystem: '<S569>/SinkBlock'
  ...
  // End of Terminate for MATLABSystem: '<S569>/SinkBlock'

  // Terminate for MATLABSystem: '<S585>/SinkBlock'
  ...
  // End of Terminate for MATLABSystem: '<S585>/SinkBlock'
  FLIGHT_hexa_PX4Timestamp_m_Term(&FLIGHT_hexacopter_DW.PX4Timestamp_pnae);
