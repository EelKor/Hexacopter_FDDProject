...
{
  while (runModel) {
    rt_StopDataLogging(MATFILE, FLIGHT_Hexacopter_fail_M->rtwLogInfo);
  }
...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(FLIGHT_Hexacopter_fail_M) == (NULL));
  while (runModel) {
    px4_sem_wait(&baserateTaskSem);
    FLIGHT_Hexacopter_fail_step();

    // Get model outputs here
    stopRequested = !((rtmGetErrorStatus(FLIGHT_Hexacopter_fail_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(FLIGHT_Hexacopter_fail_M, "stopping the model");
  runModel = 0;
}
...
void *terminateTask(void *arg)
{
  ...
  // Terminate model
  FLIGHT_Hexacopter_fail_terminate();
...
int px4_simulink_app_task_main (int argc, char *argv[])
{
  ...
  rtmSetErrorStatus(FLIGHT_Hexacopter_fail_M, 0);

  // Initialize model
  FLIGHT_Hexacopter_fail_initialize();
