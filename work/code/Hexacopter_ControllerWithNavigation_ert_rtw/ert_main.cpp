...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(Hexacopter_ControllerWithNavigation_M) == (NULL));
  while (runModel) {
    px4_sem_wait(&baserateTaskSem);
    Hexacopter_ControllerWithNavigation_step();

    // Get model outputs here
    stopRequested = !((rtmGetErrorStatus(Hexacopter_ControllerWithNavigation_M) ==
                       (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(Hexacopter_ControllerWithNavigation_M, "stopping the model");
  runModel = 0;
}
...
void *terminateTask(void *arg)
{
  ...
  // Terminate model
  Hexacopter_ControllerWithNavigation_terminate();
...
int px4_simulink_app_task_main (int argc, char *argv[])
{
  ...
  rtmSetErrorStatus(Hexacopter_ControllerWithNavigation_M, 0);

  // Initialize model
  Hexacopter_ControllerWithNavigation_initialize();
