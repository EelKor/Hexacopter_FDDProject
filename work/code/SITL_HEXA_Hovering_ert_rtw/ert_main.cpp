...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(SITL_HEXA_Hovering_M) == (NULL));
  while (runModel) {
    px4_sem_wait(&baserateTaskSem);
    SITL_HEXA_Hovering_step();

    // Get model outputs here
    stopRequested = !((rtmGetErrorStatus(SITL_HEXA_Hovering_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(SITL_HEXA_Hovering_M, "stopping the model");
  runModel = 0;
}
...
{
  rtmSetErrorStatus(SITL_HEXA_Hovering_M, 0);

  // Initialize model
  SITL_HEXA_Hovering_initialize();
