...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(Copy_of_HITL_HEXA_MISSION_M) == (NULL));
  while (runModel) {
    px4_sem_wait(&baserateTaskSem);
    Copy_of_HITL_HEXA_MISSION_step();

    // Get model outputs here
    stopRequested = !((rtmGetErrorStatus(Copy_of_HITL_HEXA_MISSION_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(Copy_of_HITL_HEXA_MISSION_M, "stopping the model");
  runModel = 0;
}
...
void *terminateTask(void *arg)
{
  ...
  // Terminate model
  Copy_of_HITL_HEXA_MISSION_terminate();
...
{
  rtmSetErrorStatus(Copy_of_HITL_HEXA_MISSION_M, 0);

  // Initialize model
  Copy_of_HITL_HEXA_MISSION_initialize();
