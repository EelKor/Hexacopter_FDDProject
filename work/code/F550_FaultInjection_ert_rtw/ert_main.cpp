...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(F550_FaultInjection_M) == (NULL));
  while (runModel) {
    px4_sem_wait(&baserateTaskSem);
    F550_FaultInjection_step();

    // Get model outputs here
    stopRequested = !((rtmGetErrorStatus(F550_FaultInjection_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(F550_FaultInjection_M, "stopping the model");
  runModel = 0;
}
...
int px4_simulink_app_task_main (int argc, char *argv[])
{
  ...
  rtmSetErrorStatus(F550_FaultInjection_M, 0);

  // Initialize model
  F550_FaultInjection_initialize();
