...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(FLIGHT_hexacopter_renew_M) == (NULL));
  while (runModel) {
    px4_sem_wait(&baserateTaskSem);
    FLIGHT_hexacopter_renew_step();

    // Get model outputs here
    stopRequested = !((rtmGetErrorStatus(FLIGHT_hexacopter_renew_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(FLIGHT_hexacopter_renew_M, "stopping the model");
  runModel = 0;
}
...
void *terminateTask(void *arg)
{
  ...
  // Terminate model
  FLIGHT_hexacopter_renew_terminate();
...
int px4_simulink_app_task_main (int argc, char *argv[])
{
  ...
  rtmSetErrorStatus(FLIGHT_hexacopter_renew_M, 0);

  // Initialize model
  FLIGHT_hexacopter_renew_initialize();
